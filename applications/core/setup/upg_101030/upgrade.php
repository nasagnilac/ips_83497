...
class _Upgrade
{
	...
	{
		\IPS\Db::i()->update( 'core_rc_index', 'last_updated=first_report_date', "last_updated=''" );
		return TRUE;
	}
	...
	{
		/* Trigger search index rebuild - we had to drop an index in core_search_index so
			queries.json truncates the table and we have to rebuild it here */
		if ( !\IPS\Request::i()->extra )
		{
		...
		foreach( \IPS\Db::i()->select( '*', 'core_rc_index', null, 'id ASC', array( $limit, $perCycle ) ) as $report )
		{
			...
			try
			{
				$item	= ( in_array( 'IPS\Content\Comment', class_parents( $item ) ) ) ? $item->item() : $item;
